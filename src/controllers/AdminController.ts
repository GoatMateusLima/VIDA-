...
export class AdminController {
  ...
  static async applyForVolunteer(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      if (!req.user) {
      ...
      const schema = z.object({
        motivation: z.string().min(10, 'Escreva uma justificativa mais detalhada'),
        experience: z.string().min(5, 'Descreva sua experiência ou treinamento anterior'),
      });

      const body = schema.parse(req.body);
      const data = await VolunteerService.apply(req.user.id, body.motivation, body.experience);
  ...
  static async approveVolunteer(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      if (!req.user) {
        res.status(401).json({ status: 'error', message: 'Não autenticado' });
        return;
  ...
  static async updateAvailability(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      const body = schema.parse(req.body);
      const data = await VolunteerService.updateStatus(req.user.id, body.status);
